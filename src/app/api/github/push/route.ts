...
export async function POST(req: NextRequest) {
  ...
  const githubOwner = user?.githubUsername || process.env.GITHUB_OWNER || 'ashishpropt'

  try {
    const repoUrl = await pushProjectToGitHub(githubToken, githubOwner, project.name, {
