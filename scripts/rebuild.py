...
# Preserve existing .env keys, only write if missing
sftp = client.open_sftp()
try:
    sftp.stat('/opt/ardoura-ai/.env')
    print("\n[.env] Already exists — keeping existing keys")
...
steps = [
    ...
    ("npm install", "cd /opt/ardoura-ai && npm install 2>&1 | tail -3"),
